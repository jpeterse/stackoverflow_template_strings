...
void template_engine::begin() {
  sep_char = '@';
}


void template_engine::setFieldSeperator( const char c ) {
  sep_char = c;
}


bool template_engine::process_line( char* line, G_MATCH_T &match ) {
  bool _retVal = false;
  MatchState ms;
...
char template_engine::hasTemplate( MatchState &ms, char* searchStr ) {
...
void template_engine::parceTemplateEntry( MatchState &ms, G_MATCH_T &match ) {
